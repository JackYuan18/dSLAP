...
def computation_time_minmax(network):
    lw=5
    MEAN_OCA=[]
    MAX_OCA=[]
    MIN_OCA=[]
    ...
    for t in range(8):
        ...
        oca=[r.oca_time[t] for r in network if t<len(r.oca_time)]
        mean_oca=np.mean(oca)
        MEAN_OCA.append(mean_oca)
        max_oca=np.max(oca)
        MAX_OCA.append(max_oca)
        min_oca=np.min(oca)
        MIN_OCA.append(min_oca)
        
        ica=[r.ica_time[t] for r in network if t<len(r.ica_time)]
        mean_ica=np.mean(ica)
        MEAN_ICA.append(mean_ica)
        max_ica=np.max(ica)
        MAX_ICA.append(max_ica)
        min_ica=np.min(ica)
        MIN_ICA.append(min_ica)
        
        learning=[r.Learning_time[t] for r in network if t<len(r.Learning_time)]
        mean_learning=np.mean(learning)
        MEAN_LEARNING.append(mean_learning)
        max_learning=np.max(learning)
        MAX_LEARNING.append(max_learning)
        min_learning=np.min(learning)
        MIN_LEARNING.append(min_learning)
        
        al=[r.AL_time[t] for r in network if t<len(r.AL_time)-1]
        print(al)
        mean_al=np.mean(al)
        MEAN_AL.append(mean_al)
        max_al=np.max(al)
        MAX_AL.append(max_al)
        min_al=np.min(al)
        MIN_AL.append(min_al)
        
    figure1=plt.figure()
    plt.plot(MEAN_OCA,'-',color='blue',label='Mean',linewidth=lw)
    plt.plot(MAX_OCA,':',color='green', label='Max/Min',linewidth=lw)
#    plt.legend()
    plt.plot(MIN_OCA,':',color='green',linewidth=lw)
    plt.xlabel('Iteration',fontsize=20)
    plt.ylabel('Time (second)',fontsize=20)
    
    
    figure2=plt.figure()
    plt.plot(MEAN_ICA,'-',color='blue',label='Mean',linewidth=lw)
    plt.plot(MAX_ICA,':',color='green', label='Max/Min',linewidth=lw)
#    plt.legend()
    plt.plot(MIN_ICA,':',color='green',linewidth=lw)
    plt.xlabel('Iteration',fontsize=20)
    plt.ylabel('Time (second)',fontsize=20)
    
    figure3=plt.figure()
    plt.plot(MEAN_LEARNING,'-',color='blue',label='Mean',linewidth=lw)
    plt.plot(MAX_LEARNING,':',color='green', label='Max/Min',linewidth=lw)
#    plt.legend()
    plt.plot(MIN_LEARNING,':',color='green',linewidth=lw)
    plt.xlabel('Iteration',fontsize=20)
    plt.ylabel('Time (second)',fontsize=20)
    
    figure4=plt.figure()
    plt.plot(MEAN_AL,'-',color='blue',label='Mean',linewidth=lw)
    plt.plot(MAX_AL,':',color='green', label='Max/Min',linewidth=lw)
#    plt.legend()
    plt.plot(MIN_AL,':',color='green',linewidth=lw)
    plt.xlabel('Iteration',fontsize=20)
    plt.ylabel('Time (second)',fontsize=20)
...
#    np.random.seed(10) 
#    final=False
    for psi in [1]:#np.linspace(0,10,30):
        robo_network=robot_network(A)
        r1=robot(np.array([[0.20,0.80,0.5]]),car_disturbance, car, robot_M, robot_l,sigma,1,robot_size)
        r2=robot(np.array([[0.80,0.80,0.5]]),car_disturbance, car,  robot_M, robot_l,sigma,2,robot_size)
        r3=robot(np.array([[0.80,0.50,-0.5]]),car_disturbance, car,  robot_M, robot_l,sigma,3,robot_size)
...
#    
#    plt.plot(np.linspace(0,10,30),np.array(Total)*0.1)
...
#                    print(r.id,min_dist)
                if len(r.X)>length:
                    length=len(r.X)
            xx=np.linspace(0,int(length*0.1),length)
...
#            plt.xticks(range(0, (length+1)*0.2))
            ...
            plt.title('minimum distance: '+str(round(min_dist,3)),fontsize=15)
            ...
            for r in robo_network.network:
                ...
                if min(r.safe_state_ratio)<min_safe:
                    
                    min_safe=min(r.safe_state_ratio)*100
#                    print(r.id,min_dist)
                if len(r.safe_state_ratio)>length:
                    length=len(r.safe_state_ratio)
            plt.plot(r1.safe_state_ratio[1:],color='g', label='robot '+str(r1.id))
            plt.plot(r2.safe_state_ratio[1:],color='blue',label='robot '+str(r2.id))
            plt.plot(r3.safe_state_ratio[1:],color='orange',label='robot '+str(r3.id))
            plt.plot(r4.safe_state_ratio[1:],color='cyan',label='robot '+str(r4.id))
            plt.plot(r5.safe_state_ratio[1:],color='purple',label='robot '+str(r5.id))
            plt.plot(r6.safe_state_ratio[1:],color='black',label='robot '+str(r6.id))
            plt.plot(r7.safe_state_ratio[1:],color='gold',label='robot '+str(r7.id))
            plt.plot(r8.safe_state_ratio[1:],color='magenta',label='robot '+str(r8.id))
            plt.xlabel('Iteration',fontsize=20)
            plt.ylabel('Number of safe states',fontsize=20)
            plt.legend()

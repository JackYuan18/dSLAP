...
def SI(x,Xfree,Up,eps_P,X_temp_tree,X_temp,mu_prep,cov_prep,train,kernel):
    den = len(Up)
    index=[]
    S=[]
...
#        unsafe = True
        ...
        mu,cov=gp(mu_prep,cov_prep,[test],train,kernel,car_true)
        z=x.state+eps_P*mu
        z=z[0]
        if z[2]>pi: 
            z[2]=z[2]-2*pi
...
#        unsafe,states = Xfree.safe_kdtree_search(X_temp,z)
        n=sqrt(cov)/hp
        inds = X_temp_tree.query(z.reshape(1,-1),int(6+ceil(6*n)),return_distance=False)
...
#        plt.ylabel('Pursuer')
    ...
    while t_start<t_end:
        u=random.sample(set(Up),1)
        car_xdot=car(car_x,u[0])
        
        train.append(np.concatenate((car_x,np.array(u))))
        train_f.append(car_xdot)
        
        t_start=t_start+t_step
        car_x=car_x+t_step*car_xdot
    train=np.stack(train)    
    train_f=np.stack(train_f)
...
#                        mu,cov=gp(kernel,s.state,train, train_f)
                        childs = SI(s,Xfree,Up,eps_p,X_temp_tree,X_temp,mu_prep,cov_prep,train,kernel)
                        s.s_=s.s
                        s.childs = childs
...
#                        print('need update')
            ...
            for x in Kp:
#                    mu,cov=gp(kernel,s.state,train, train_f)
                    SI(s,Xfree,Up,eps_p,X_temp_tree,X_temp,mu_prep,cov_prep,train,kernel)

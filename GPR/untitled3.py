...
#                    print(u_)
#                    if u_!=child.from_u:
#                        U_childs[child.from_u].add(child)
#                        Temp_set[child.from_u].add(child)                        
#                        try:
#                            U_childs[u_].remove(child)                            
#                        except KeyError:
#                            continue
#                        try:
#                            Temp_set[u_].remove(child)
#                        except KeyError:
#                            continue
                            
    else:
        for child in x.childs[u]:
#            print(child.r,x.cov[u])
            try:
                if x.cov[u]>child.r:
                    child.r=x.cov[u]
#                    if child.from_u is not None:
##                        print(changed)
##                        u_=child.from_u
#                        child.r=x.cov[u]
#                        child.from_u=u
...
def parent_update(S):
    for s in S:
        for u in s.parents_:
            for x in s.parents_[u]:
                if not x.unsafe:
                    index=np.where(x.u_==u)
                    if len(index[0])>0:
                        x.u_=np.delete(x.u_,index[0][0])
                    if len(x.u_)==0:
                        x.unsafe=True
#                        X_unsafe.append(x)
                        S.append(x)
...
def optimal_val_in_states(S,tk,goal=None):
#    s_min=None
#    min_val=float('inf')
#    u_min=None
    if goal is None:
#        min_val=min([s.to_goal for s in S])
        min_val=min([(1-exp(-tk-1))*s.to_goal-exp(-tk-1)*s.r for s in S])#exp(-tk)*s.r
    else:
        
        min_val=min([np.linalg.norm(s[0,:-1]-goal[0,:-1]) for s in S])

    return min_val
def optimal_control(U_childs,tk,goal=None):
    min_val=float('inf')
    u_min=None
    for u in U_childs:
        if len(U_childs[u])>0:
            min_val_=optimal_val_in_states(U_childs[u],tk,goal)
            if min_val_<min_val:
                min_val=min_val_
                u_min=u
    return u_min
...
#        self.r_iota=0
    ...
    def Reach(self,robot):
#        states=set()
        for ind,u in enumerate(self.Up):
            x=robot.x.reshape((3,))
            mu,cov=robot.gp_local_single(np.concatenate((x,np.array([u]))))
            z=x+robot.eps_p*(mu+robot.car(x,u))
            
            std=np.max(np.sqrt(cov))
            alpha_p=2*robot.hp+robot.l*robot.eps_p*robot.hp+robot.M*robot.l*robot.eps_p**2+0.5*(self.M*self.eps_p+self.hp)
            z=z.reshape(1,-1)
            inds1 = self.X_temp_reduced_tree.query_radius(z[:,:-1],0.5*alpha_p+2*self.size+robot.gamma*std)#aplt.gca().add_patch(circle1)lpha_p+self.gamma*std
    ...
    def SI(self,x,alpha_p):
        ...
        for ind,u in enumerate(self.Up):
            ...
            while z[2]<-1:
                z[2] = z[2]+2
            z=z.reshape(1,-1)
            inds1 = self.X_temp_tree.query_radius(z,0.5*alpha_p)
            theta=z[0,2]
            if theta<-1+0.5*alpha_p:
                z[0,2]=1
                inds2 = self.X_temp_tree.query_radius(z,-theta+0.5*alpha_p-1)
                inds=np.concatenate((inds1[0],inds2[0]))
            elif theta>1-0.5*alpha_p:
                z[0,2]=-1
                inds2 = self.X_temp_tree.query_radius(z,-theta-0.5*alpha_p+1)
                inds=np.concatenate((inds1[0],inds2[0]))
            else:
    ...
    def SI_GP(self, x,X_temp_tree,X_temp,hp):
        index=[]    
        for ind,u in enumerate(self.Up):
            mu,cov=self.gp_local_single(np.concatenate((x.state,np.array([u]))))
#            mu=-0.015
#            cov=0
            x.cov_[u]=np.max(cov)
            x.mu_[u]=mu
            z=x.state+self.eps_p*(mu+self.car(x.state,u))
            z=z[0]
            while z[2]>1: 
                z[2]=z[2]-2
            while z[2]<-1:
                z[2] = z[2]
            std=np.max(x.cov_[u])
            alpha_p=2*self.hp+self.l*self.eps_p*self.hp+self.M*self.l*self.eps_p**2+2*self.gamma*std
            z=z.reshape(1,-1)
            inds1 = X_temp_tree.query_radius(z,0.5*alpha_p)
            theta=z[0,2]
            if theta<-1+0.5*alpha_p:
                z[0,2]=1
                inds2 = X_temp_tree.query_radius(z,-theta+0.5*alpha_p-1)
                inds=np.concatenate((inds1[0],inds2[0]))
            elif theta>1-0.5*alpha_p:
                z[0,2]=-1
                inds2 = X_temp_tree.query_radius(z,-theta-0.5*alpha_p+1)
...
#                x.U[u]=0
    ...
    def inter_robot_collision_avoidance(self,robo_network):
        ...
        if state_temp is not None and len(state_temp)>0:
            ...
            self.X_safe_=KDTree(X_safe)
            
            self.X_safe_set=X_safe
    def obstacle_collision_avoidance_GP(self, obs,p):
#        print(p)  
...
#        ball_obs_1 = ball(obs_1,0.5*(self.M*self.eps_p+self.hp)+self.size)#self.M*self.eps_p+self.hp

        #table construction based on dynamics
        need_update=[]
#        start=time.time()
        X_temp,state_temp, X_temp_reduced = gridsearch_construct(self.Xfree,p,self.Up,obs=ball_obs) #identify obstacle unsafe state
#        print(time.time()-start)
        X_temp_tree=KDTree(X_temp)
        self.X_temp=X_temp
        self.X_temp_tree=X_temp_tree
        ...
        for s in state_temp:
            if not s.unsafe:   
                    self.SI_GP(s,X_temp_tree,X_temp,self.hp)               
                    if s.unsafe:                               
...
#        X_safe= [x.state for x in state_temp if not x.unsafe]
        print('after oca:',len(safe_states))
#        if len(X_safe)>0:
#            self.X_safe=KDTree(X_safe)
#            self.X_safe_set=X_safe
        self.state_temp=state_temp #graphing purpose
...
#                safe_states.add(x)
#        X_safe= [x.state for x in self.state_temp if not x.unsafe]
        safe_states=set()
        X_safe=[]
                ...
                for u in x.u:
                    for s in Next_childs[u]:
                        for v in s.u:
                            ...
                            Temp_set[u]=Temp_set[u].union(s.childs[v])

                for u in x.u:
                    for s in Next_childs[u]:
                        for v in s.u:
                            Temp_set=assign_learning_utility(s,v,U_childs,Temp_set,aggregate=True)
                                
                Next_childs=Temp_set
            x.u_min=optimal_control(U_childs,tk)
...
#            print(self.x[0,2])
    ...
    def run_and_sample_MultiD(self,tk,network,obs):
        if np.linalg.norm(self.x[0,:-1]-self.goal[0,:-1])>0.05 and np.max(self.x[0,:-1])<2 and np.min(self.x[0,:-1])>-1:
            if self.X_safe is None:
                ...
                ind=random.randint(0,len(self.Up)-1) 
                u=self.Up[ind]
            else:
#                if self.x[0,1]>1 or self.x[0,1]<0 or self.x[0,0]>1 or self.x[0,0]<0:
#                    u=-0.3#self.active_learning_x(2,tk)
#                else:
                u=self.active_learning(2,tk)
            if u is None:
                u=-0.3#self.active_learning_mu(2,tk)
...
#            print('to robots:',dist)
            if dist<self.size*2:
                ...
                if len(self.Y)>1:
        #            print('Y=',len(self.Y))
                    ind=random.randint(0,len(self.Y)-2) 
        ...
        if np.linalg.norm(self.x[0,:-1]-self.goal[0,:-1])>0.05 and np.max(self.x[0,:-1])<2 and np.min(self.x[0,:-1])>-1:
            if self.X_safe is None:
                ...
                if self.x[0,1]>1 or self.x[0,1]<0 or self.x[0,0]>1 or self.x[0,0]<0:
                    ...
                    u=self.active_learning(2,tk)
            if u is None:
                u=self.active_learning_mu(2,tk)
...
#        self.sigma=sigma
        self.gpr=None
#   
    def control_init(self, car,car_disturbance, X_dim,M,l,Up,obs,P):
        for robot in self.network:
            robot.car=car
            robot.dynamics=car_disturbance
            Xfree=grid(X_dim)
            robot.Xfree=Xfree.construct(P,Up,robot.goal)
            robot.M=M
            robot.l=l
            robot.Up=Up
            robot.obstacle_collision_avoidance(obs,P)
    def safe_planner(self,obs_1,p,GP=False):
        for robot in self.network:
            if not robot.stop:
                print('robot'+str(robot.id))
                if GP:
                    robot.obstacle_collision_avoidance_GP(obs_1,p)
                else:
...
#                        robot.dataR[str(zr)][d]=(next(mu_iter),next(cov_iter))
    ...
    def run_and_sample_MultiD(self,tk,obs):
        for robot in self.network:
            if not robot.stop:
                robot.run_and_sample_MultiD(tk,self.network,obs)
...
#            ZZ.append(self.data[str(x)][0])def run_and_sample(self,eta):
    ...
    #        plt.legend(prop={'size': 15})
        ...
        return sumError
def get_Z_M(linspaceX,linspaceU):
    Z_M=[]
    for i in linspaceX:
        for j in linspaceX:
            for l in linspaceX:
                for u in linspaceU:
                    Z_M.append(np.array([i,j,l,u]))
    return np.array(Z_M)
...
def print_result(robo_network,robo,T,M,sumError_local,sumError_fuse,plot,eta1, Z_R):
    
    r1=robo_network.network[robo]
    ZZ=[]
    N=len(r1.data_keys)
    for x in r1.data_keys:    
        truth=eta1(x)
...
#    robo_network.gpr_prep()
#
def learning_MultiD_no_communication(robo_network,kernel,tk,obs,T=100):
#    for robot in robo_network.network:
#        robot.X_safe=robot.X_safe_
    
    for t in range(T):
#        print(t)
        robo_network.run_and_sample_MultiD(tk,obs)
    robo_network.gpr_prep()
#        print('t=',t, 'gpr local')
